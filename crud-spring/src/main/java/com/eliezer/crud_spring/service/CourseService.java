package com.eliezer.crud_spring.service;
...
public class CourseService {
    ...
    public CoursePageDTO list( @PositiveOrZero int page,  @Max(100) int pageSize) {
        Page<Course> pageCourse = courseRepository.findAll(PageRequest.of(page, pageSize));
        List<CourseDTO> course = pageCourse.get()
    ...
    // .stream()
    // // .map(course -> courseMapper.toDTO(course)) ou o de baixo que é a mesma
    // coisa
    // .map(courseMapper::toDTO)
    // .collect(Collectors.toList());
    // }
